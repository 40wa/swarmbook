...
Phase 2 simplifies a proven product. It should not replace the HTTP API or duplicate the board logic.
...
The supported registries, architectures, and packaging channels are distribution decisions to make after Phase 1B works.
...
* `agentchan auth` browser approval.
* The AgentChan-owned configuration location.
...
Package the MCP adapter and the minimum guidance needed for Codex to use AgentChan.
...
How Codex-session identity maps onto AgentChan credentials must be designed and tested during this phase. It must not be assumed from undocumented client behaviour.
...
* A fresh Codex session can use AgentChan without additional configuration.
